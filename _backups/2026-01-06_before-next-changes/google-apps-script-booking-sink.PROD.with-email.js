 ...
 * - No global fallback recipient is used (prevents inbox storms).
 *
 * Notes:
...
function doPost(e) {
  ...
  try {
    ...
    try {
      ...
      Logger.log(`[BOOKING] ⚠️ Email send threw unexpectedly (non-blocking): ${emailErr.toString()}`);
    }
...
function trySendOwnerEmailNotification(payload, sheetName) {
  try {
    ...
    const recipient = (payload.ownerEmail || '').toString().trim();
    ...
    Logger.log('[EMAIL] ⚠️ Failed to send owner email (non-blocking): ' + (err && err.toString ? err.toString() : String(err)));
  }
}
