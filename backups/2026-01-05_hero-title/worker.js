...
export default {
  async fetch(request, env, ctx) {
    ...
    if (propertySlug) {
      ...
      const html = renderSmartTemplate(config, template, designProfile);
...
function renderSmartTemplate(config, templateHTML, designProfile) {
  ...
  const heroTextStyleAttr = hasHeroTextStyle
    ? `style="` +
      (heroTextColor ? `--hero-text-color:${heroTextColor}; ` : '') +
  ...
  const heroTitleModeRaw = String(
    config.heroTitleMode ?? config.branding?.heroTitleMode ?? 'auto'
  ).trim().toLowerCase();
  const heroTitleMode = (heroTitleModeRaw === 'show' || heroTitleModeRaw === 'hide' || heroTitleModeRaw === 'auto')
    ? heroTitleModeRaw
    : 'auto';
  ...
  const shouldRenderHeroTitle = heroTitleMode === 'hide'
    ? false
    : heroTitleMode === 'show'
      ? true
      : !(heroImageContainsBrand && has_logo);
  const has_hero_title = Boolean(shouldRenderHeroTitle && heroTitleText);
  ...
  let html = templateHTML
    ...
    .replace(/{{HERO_IMAGE}}/g, (config.heroImage || config.branding?.heroImage || 'https://images.unsplash.com/photo-1566073771259-6a8506099945') + (config.heroImage && config.heroImage.includes('img.webzyl.com') ? '?w=1920&q=90' : ''))
